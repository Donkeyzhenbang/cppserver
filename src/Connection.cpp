...
Connection::Connection(EventLoop *loop, Socket *sock)
    : loop_(loop), sock_(sock), channel_(nullptr), inBuffer(new std::string()), readBuffer(nullptr) {
    ...
    while (true) {  //由于使用非阻塞IO，读取客户端buffer，一次读取buf大小数据，直到全部读取完毕
        ...
        if (bytes_read > 0) {
            ...
            deleteConnectionCallback(
                sockfd);  //!注意这里会执行客户端断开连接的回调函数，会点函数注册在Server类中newConnection中
            break;
        } else {
...
void Connection::setDeleteConnectionCallback(std::function<void(int)> cb) { deleteConnectionCallback = cb; }
