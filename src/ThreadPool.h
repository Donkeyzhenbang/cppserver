...
public:
    ...
    template<typename F, typename... Args> //F 是类型，f 是该类型的对象。用户可能误以为 F 直接代表可调用对象，而非其类型。
    auto add(F&& f, Args&&... args) //!注意这里模板类是 T&&是万能引用，可以接收左值或右值 F&& 允许完美转发（Perfect Forwarding），无论传递的是左值还是右值，都能保留值类别。
...
auto ThreadPool::add(F&& f, Args&&... args) -> std::future<typename std::result_of<F(Args...)>::type> {
    ...
    //std::packaged_task 是一个模板类，它包装了一个可调用对象（如函数、Lambda 等），允许在之后的某个时间点异步地执行这个任务并获取返回值。这里的 return_type() 表示任务没有参数，只返回类型 return_type。
    //std::bind 将 f 和参数 args... 绑定成一个新的可调用对象，使得 f 可以在以后被调用，且参数已经被预先绑定。std::forward<F>(f) 和 std::forward<Args>(args)... 是完美转发，确保参数的左值或右值属性得到保持，避免不必要的拷贝或移动。
    ...
    //get_future() 是 std::packaged_task 提供的一个方法，目的是 从 std::packaged_task 中获取一个 std::future 对象。通过这个 std::future 对象，调用者能够在未来某个时刻 获取任务的执行结果。
